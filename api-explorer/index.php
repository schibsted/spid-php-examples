...
if (!$spidClientConfig) {
    die('ERROR: You must configure $spidClientConfig  in config.php first!');
}
...
<head>
    ...
    <script>
        function getEndpointName(endpoint) {
            return endpoint.category.slice(-1).pop() + ": " + endpoint.name;
        }
    </script>
    <script type="text/javascript" src="js/prettify.js"></script>
    <title>SPiD API Explorer</title>
</head>
...
<script id="methodsTemplate" type="text/x-jquery-tmpl">
    ...
    {{each(i, item) methods}}
        <option value="${i}">${getEndpointName(item)}</option>
    {{/each}}
    </select>
</script>
